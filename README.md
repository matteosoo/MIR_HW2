...
## Q7
### IDEA
針對音樂上這樣[複節拍（polymeter）](https://zh.wikipedia.org/wiki/节拍#复节拍（混合节拍）)，又或是會被俗稱「變態拍」這部分，設計演算法上最重要應該在於掌握他的動態變化。
而首先我認為前處理上，可以參考mireval官方library中的文件有提到，利用“*mireval.beat.trimbeats*”作為data preprocessing去除音樂前五秒的片段以利提升資料的正確性。
此外，適當的去除開頭的「弱起拍」節奏，以前消除尾巴不完整的小節拍數，可有效從正確位置上開始被偵測到結束。
